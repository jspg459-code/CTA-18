...
export default function Home(){
  ...
  return <main className="gamePage">
    ...
    {creatorPanel}
    {activeTab==='map'?<><section className="mapShell"><OperationalMap stations={operationalStations} fallback={departmentCenter(selectedService.code)} activeIntervention={activeCall} vehicles={engagedVehicles} callAccepted={callAccepted} onTakeCall={takeCall} onRefuseCall={refuseCall}/>{activeCall&&!callAccepted&&workflow==='map'&&<button className="answerCallButton" onClick={takeCall}><span>📞</span><div><small>APPEL ENTRANT</small><b>RÉPONDRE À L'APPEL</b><em>{activeCall.scenario?.title||'Nouvelle intervention'}</em></div></button>}{stationsLoading&&<div className="mapMessage">Chargement des centres…</div>}{stationsError&&<div className="mapMessage error">{stationsError}</div>}</section>
