...
const operatorConsole = selectedService ? <div className="operatorConsoleInline">
  ...
  {operatorPanel==='dispatch' && <div className="operatorBody dispatchWorkspace">
    ...
    {!activeCall?<div className="operatorEmpty">Aucune intervention sélectionnée.</div>:stationsLoading?<div className="operatorEmpty">⏳ Chargement des CIS disponibles…</div>:stationsError?<div className="operatorEmpty"><p>⚠️ {stationsError}</p><button className="operatorPrimary" onClick={()=>{setStationsError('');setStationsLoading(true);fetch('/api/stations?code='+encodeURIComponent(selectedService.code)+'&retry='+Date.now(),{cache:'no-store'}).then(r=>r.json()).then(data=>{if(Array.isArray(data.stations)&&data.stations.length){setStations(data.stations);setStationsError('')}else setStationsError(data.error||'Aucun centre retourné.');}).catch(()=>setStationsError('Impossible de charger les CIS.')).finally(()=>setStationsLoading(false));}}>🔄 Réessayer</button></div>:operationalStations.length===0?<div className="operatorEmpty"><p>⚠️ Aucun CIS disponible pour le moment.</p><button className="operatorPrimary" onClick={()=>window.location.reload()}>🔄 Recharger les centres</button></div>:<>
      <div className="dispatchBriefGrid">
        <section className="dispatchBriefCard"><h4>🚨 Intervention</h4><strong>{activeCall.scenario.title}</strong><p>{activeCall.address}</p><div className="dispatchBriefTags"><span>{activeCall.scenario.category}</span><span>{activeCall.scenario.difficulty}</span>{activeCall.scenario.victimTransport?.has_victims&&<span>🏥 {activeCall.scenario.victimTransport.victim_count||0} victime(s)</span>}</div></section>
        <section className="dispatchBriefCard required"><h4>🚒 Moyens indispensables</h4><div className="requiredVehicleList">{activeCall.scenario.requiredVehicles.length?activeCall.scenario.requiredVehicles.map(v=><b key={v}>{v}</b>):<span>Aucun moyen imposé</span>}</div><small>Les CIS sont triés par distance réelle par rapport au lieu de l'intervention.</small></section>
      </div>
      <div className="dispatchFilterBar"><button className="active">Tous les moyens</button><button>Indispensables</button><button>Recommandés</button><button>Renforts</button><span>{operationalStations.length} CIS disponibles</span></div>
      <div className="dispatchTable">
        <div className="dispatchTableHead"><span>Centre</span><span>Véhicule</span><span>Rôle</span><span>Pompiers</span><span>Distance</span><span></span></div>
        <div className="dispatchTableBody">{operationalStations.slice(0,80).flatMap(st=>{const distance=Number(st.distanceToInterventionKm);return (st.fleet||[]).flatMap(v=>Array.from({length:v.count||1}).map((_,i)=>({st,v,i,distance})));}).map(({st,v,i,distance})=>{const required=activeCall.scenario.requiredVehicles.includes(v.type);const recommended=activeCall.scenario.recommendedVehicles.includes(v.type);return <div className={'dispatchTableRow'+(required?' requiredRow':'')} key={st.id+'-'+v.type+'-'+i}><div><b>{st.name}</b><small>{st.address||'CIS référencé'}</small></div><strong className="vehicleChip">🚒 {v.type} {i+1}</strong><span className={'dispatchRole '+(required?'critical':recommended?'recommended':'')}>{required?'Indispensable':recommended?'Recommandé':'Disponible'}</span><span>👨‍🚒 {Math.max(2,Math.min(6,st.personnel||3))}</span><span>{Number.isFinite(distance)?(distance<1?Math.round(distance*1000)+' m':distance.toFixed(1)+' km'):'—'}</span><button className="dispatchSendButton" onClick={()=>dispatchVehicle(st,v.type)}>Engager</button></div>})}</div>
      </div>
      <div className="dispatchBottomBar"><div><b>{dispatchVehicles.length} véhicule(s) engagé(s)</b><span>Les engagements apparaissent immédiatement sur la carte et dans l'intervention en cours.</span></div><button className="operatorPrimary" onClick={()=>setCtaView('map')}>🗺️ Voir la carte opérationnelle</button></div>
    </>}</div>}
